...
export default class LineCharts extends Component {
    ...
    async fetchData(dateNew) {
        ...
        let apiURL = new URL(`${this.state.apiKey}_date`, baseURL);
        await fetch(apiURL)
            .then(response => response.json())
            .then(resp => {
                for (var key in resp.data) {
                    dateNew = new Date(this.state.newDate).toLocaleDateString();
                    selectedDate1 = (new Date(resp.data[key].day).toLocaleDateString());
                    if (dateNew === selectedDate1) {
                        value.push(resp.data[key].value);
                        labels.push(resp.data[key].time);
                    }
                }
                this.setState({
        ...
        return (
            <ScrollView>
                ...
                <View style={styles.wrapper}>
                    <Text style={styles.tit}>Thống kê theo ngày</Text>
                    <LineChart
                        data={{

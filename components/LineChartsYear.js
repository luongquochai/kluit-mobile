...
export default class LineCharts extends Component {
    ...
    async fetchData(dateNew) {
        ...
        let apiURL = new URL(`${this.state.apiKey}_year`, baseURL);
        await fetch(apiURL)
            ...
            .then(resp => {
                for (var key in resp.data) {
                    ...
                    selectedYear.push(resp.data[key].year);
                    // value.push(resp.data[key].value);
                    // labels.push(resp.data[key].day);
                }
                yearpicker = new Date(dateNew).getFullYear();
                selectedYear = selectedYear.filter(function (e, i, self) {
                    return i === self.indexOf(e);
                })
                for (let i = 0; i < selectedYear.length; i++) {
                    if (selectedYear[i] === yearpicker) {
                        for (var key in resp.data) {
                            //console.log(resp.data[key].month);
                            if (resp.data[key].year === selectedYear[i]) {
                                value.push(resp.data[key].value);
                                labels.push(resp.data[key].month);
                            }
                        }
                    }
                }
                this.setState({
                    //pickedDate: selectedYear,
                    val: value,
                    label: labels,
                })
            })
        ...
        return (
            <ScrollView>
                ...
                <View style={styles.wrapper}>
                    <Text style={styles.tit}>Thống kê theo năm</Text>
                    <LineChart
                        data={{
